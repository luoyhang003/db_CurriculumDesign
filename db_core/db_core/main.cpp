...
public:
    //构造函数
    Digraph(int size=DEFAULT_VERTEX_NUMBER);
    //析构函数
    ~Digraph(void);
    //判断图是否为空
    bool IsEmpty(void) const{return NumVertices==0;}
...
//构造函数Digraph()
template <class VertexType, class EdgeType>
Digraph<VertexType, EdgeType>::Digraph(int size):
NumVertices(0),MaxNumVertices(size),NumEdges(0)
{
    int n,e,k,j;    //n表示定点数，e表示边数
    VertexType newVertex,first,second;
...
//析构函数
template <class VertexType, class EdgeType>
Digraph<VertexType, EdgeType>::~Digraph(void)
{
    for (int i = 0; i < NumVertices; i++) {
        Edge<EdgeType> *p = VertexList[i].out;
        while (p!=NULL) {
...
//成员函数ReturnVertexPos()
template <class VertexType, class EdgeType>
int Digraph<VertexType, EdgeType>::ReturnVertexPos(const VertexType &vertex)
{
    for(int i = 0; i < NumVertices; i++)
    {
        if (VertexList[i].data == vertex) {
...
//成员函数ReturnFirstNeighbor()
template <class VertexType, class EdgeType>
int Digraph<VertexType, EdgeType>::ReturnFirstNeighbor(int v)
{
    Edge<EdgeType> *p;
    if (v != -1) {
        p = VertexList[v].out;
...
//成员函数ReturnNextNeighbor()
template <class VertexType, class EdgeType>
int Digraph<VertexType, EdgeType>::ReturnNextNeighbor(int vi, int vj)
{
    if (vi != -1) {
        Edge<EdgeType> *p = VertexList[vi].out;
        while (p != NULL) {
...
//成员函数ReturnEdgeInfo()
template <class VertexType, class EdgeType>
EdgeType Digraph<VertexType, EdgeType>::ReturnEdgeInfo(int vi, int vj)
{
    vi = ReturnVertexPos(vi);
    vj = ReturnVertexPos(vj);
    if (vi != -1 && vj != -1) {
...
//成员函数InsertVertex()
template <class VertexType, class EdgeType>
void Digraph<VertexType,EdgeType>::InsertVertex(const VertexType &vertex)
{
    int i = ReturnVertexPos(vertex);
    if (i > 0 && i < NumVertices) {
        cerr << "表中已有该节点" << endl;
...
//假设与该顶点关联的边都已经删除
template <class VertexType, class EdgeType>
void Digraph<VertexType, EdgeType>::DeleteVertex(int v)
{
    if (v < 0 || v > NumVertices - 1) {
        cerr<<"非法删除"<<endl;
        return;
...
//成员函数InsertEdge()
template <class VertexType, class EdgeType>
void Digraph<VertexType, EdgeType>::InsertEdge(int vi, int vj, EdgeType edgeInfo)
{
    Edge<EdgeType> *p = VertexList[vi].out;
    Edge<EdgeType> *NewEdge = NULL;
    NewEdge = new Edge<EdgeType>(vj, edgeInfo);
...
//成员函数DeleteEdge
template <class VertexType, class EdgeType>
void Digraph<VertexType, EdgeType>::DeleteEdge(int vi, int vj)
{
    Edge<EdgeType> *r;
    Edge<EdgeType> *p = VertexList[vi].out;
    if (p == NULL) {
...
//成员函数ReturnDegree
template <class VertexType, class EdgeType>
int Digraph<VertexType, EdgeType>::ReturnDegree(int v)
{
    int vi = ReturnVertexPos(v);
    int count = 0;
    if (vi != -1) {
...
public:
    //构造函数
    WeightedGraph(int size=DEFAULT_VERTEX_NUMBER);
    //析构函数
    ~WeightedGraph(void);
    //判断图是否为空
    bool IsEmpty(void) const{return NumVertices==0;}
...
//构造函数WeightedGraph()
template <class VertexType, class EdgeType>
WeightedGraph<VertexType, EdgeType>::WeightedGraph(int size):
NumVertices(0),MaxNumVertices(size),NumEdges(0)
{
    int n,e,k,j;    //n表示定点数，e表示边数
    VertexType newVertex,first,second;
...
//析构函数
template <class VertexType, class EdgeType>
WeightedGraph<VertexType, EdgeType>::~WeightedGraph(void)
{
    for (int i = 0; i < NumVertices; i++) {
        Edge<EdgeType> *p = VertexList[i].out;
        while (p!=NULL) {
...
//成员函数ReturnVertexPos()
template <class VertexType, class EdgeType>
int WeightedGraph<VertexType, EdgeType>::ReturnVertexPos(const VertexType &vertex)
{
    for(int i = 0; i < NumVertices; i++)
    {
        if (VertexList[i].data == vertex) {
...
//成员函数ReturnFirstNeighbor()
template <class VertexType, class EdgeType>
int WeightedGraph<VertexType, EdgeType>::ReturnFirstNeighbor(int v)
{
    Edge<EdgeType> *p;
    if (v != -1) {
        p = VertexList[v].out;
...
//成员函数ReturnNextNeighbor()
template <class VertexType, class EdgeType>
int WeightedGraph<VertexType, EdgeType>::ReturnNextNeighbor(int vi, int vj)
{
    if (vi != -1) {
        Edge<EdgeType> *p = VertexList[vi].out;
        while (p != NULL) {
...
//成员函数ReturnEdgeInfo()
template <class VertexType, class EdgeType>
EdgeType WeightedGraph<VertexType, EdgeType>::ReturnEdgeInfo(int vi, int vj)
{
    vi = ReturnVertexPos(vi);
    vj = ReturnVertexPos(vj);
    if (vi != -1 && vj != -1) {
...
//成员函数InsertVertex()
template <class VertexType, class EdgeType>
void WeightedGraph<VertexType,EdgeType>::InsertVertex(const VertexType &vertex)
{
    int i = ReturnVertexPos(vertex);
    if (i > 0 && i < NumVertices) {
        cerr << "表中已有该节点" << endl;
...
//假设与该顶点关联的边都已经删除
template <class VertexType, class EdgeType>
void WeightedGraph<VertexType, EdgeType>::DeleteVertex(int v)
{
    if (v < 0 || v > NumVertices - 1) {
        cerr<<"非法删除"<<endl;
        return;
...
//成员函数InsertEdge()
template <class VertexType, class EdgeType>
void WeightedGraph<VertexType, EdgeType>::InsertEdge(int vi, int vj, EdgeType edgeInfo)
{
    Edge<EdgeType> *p = VertexList[vi].out;
    Edge<EdgeType> *NewEdge = NULL;
    NewEdge = new Edge<EdgeType>(vj, edgeInfo);
...
//成员函数DeleteEdge
template <class VertexType, class EdgeType>
void WeightedGraph<VertexType, EdgeType>::DeleteEdge(int vi, int vj)
{
    Edge<EdgeType> *r;
    Edge<EdgeType> *p = VertexList[vi].out;
    if (p == NULL) {
...
//成员函数ReturnDegree
template <class VertexType, class EdgeType>
int WeightedGraph<VertexType, EdgeType>::ReturnDegree(int v)
{
    int vi = ReturnVertexPos(v);
    int count = 0;
    if (vi != -1) {
...
public:
    //构造函数
    WeightedDigraph(int size=DEFAULT_VERTEX_NUMBER);
    //析构函数
    ~WeightedDigraph(void);
    //判断图是否为空
    bool IsEmpty(void) const{return NumVertices==0;}
    //判断图的存储是否溢出
    bool isFull(void) const{return NumVertices==MaxNumVertices;}
...
//构造函数WeightedDigraph()
template <class VertexType, class EdgeType>
WeightedDigraph<VertexType, EdgeType>::WeightedDigraph(int size):
NumVertices(0),MaxNumVertices(size),NumEdges(0)
{
    int n,e,k,j;    //n表示定点数，e表示边数
    VertexType newVertex,first,second;
...
//析构函数
template <class VertexType, class EdgeType>
WeightedDigraph<VertexType, EdgeType>::~WeightedDigraph(void)
{
    for (int i = 0; i < NumVertices; i++) {
        Edge<EdgeType> *p = VertexList[i].out;
        while (p!=NULL) {
...
//成员函数ReturnVertexPos()
template <class VertexType, class EdgeType>
int WeightedDigraph<VertexType, EdgeType>::ReturnVertexPos(const VertexType &vertex)
{
    for(int i = 0; i < NumVertices; i++)
    {
        if (VertexList[i].data == vertex) {
...
//成员函数ReturnFirstNeighbor()
template <class VertexType, class EdgeType>
int WeightedDigraph<VertexType, EdgeType>::ReturnFirstNeighbor(int v)
{
    Edge<EdgeType> *p;
    if (v != -1) {
        p = VertexList[v].out;
...
//成员函数ReturnNextNeighbor()
template <class VertexType, class EdgeType>
int WeightedDigraph<VertexType, EdgeType>::ReturnNextNeighbor(int vi, int vj)
{
    if (vi != -1) {
        Edge<EdgeType> *p = VertexList[vi].out;
        while (p != NULL) {
...
//成员函数ReturnEdgeInfo()
template <class VertexType, class EdgeType>
EdgeType WeightedDigraph<VertexType, EdgeType>::ReturnEdgeInfo(int vi, int vj)
{
    vi = ReturnVertexPos(vi);
    vj = ReturnVertexPos(vj);
    if (vi != -1 && vj != -1) {
...
//成员函数InsertVertex()
template <class VertexType, class EdgeType>
void WeightedDigraph<VertexType,EdgeType>::InsertVertex(const VertexType &vertex)
{
    int i = ReturnVertexPos(vertex);
    if (i > 0 && i < NumVertices) {
        cerr << "表中已有该节点" << endl;
...
//假设与该顶点关联的边都已经删除
template <class VertexType, class EdgeType>
void WeightedDigraph<VertexType, EdgeType>::DeleteVertex(int v)
{
    if (v < 0 || v > NumVertices - 1) {
        cerr<<"非法删除"<<endl;
        return;
...
//成员函数InsertEdge()
template <class VertexType, class EdgeType>
void WeightedDigraph<VertexType, EdgeType>::InsertEdge(int vi, int vj, EdgeType edgeInfo)
{
    Edge<EdgeType> *p = VertexList[vi].out;
    Edge<EdgeType> *NewEdge = NULL;
    NewEdge = new Edge<EdgeType>(vj, edgeInfo);
...
//成员函数DeleteEdge
template <class VertexType, class EdgeType>
void WeightedDigraph<VertexType, EdgeType>::DeleteEdge(int vi, int vj)
{
    Edge<EdgeType> *r;
    Edge<EdgeType> *p = VertexList[vi].out;
    if (p == NULL) {
...
//成员函数ReturnDegree
template <class VertexType, class EdgeType>
int WeightedDigraph<VertexType, EdgeType>::ReturnDegree(int v)
{
    int vi = ReturnVertexPos(v);
    int count = 0;
    if (vi != -1) {
